...
public:
    inline void RenderSettings()
    {
        ...
        ImGui::SameLine();
        ImGui::Combo("##Demo", &m_SelectedDemoIndex, &DemoSelector::ComboGetter, &m_DemoNames, static_cast<int>(m_DemoNames.size()));
    ...
    {
        return m_SelectedDemoIndex;
    }

    inline Demo* GetSelectedDemo(Window& window, Camera& camera) const
    {
        return DemoRegistry::Instance().Create(m_DemoNames[m_SelectedDemoIndex], window, camera);
    }
