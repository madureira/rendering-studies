...
conan install .. --output-folder=./Debug --build=missing --settings=build_type=$BUILD_TYPE

# Run CMake to generate the build system in the current directory (build/)
cmake .. -B Debug  -DCMAKE_BUILD_TYPE=$BUILD_TYPE -DCMAKE_TOOLCHAIN_FILE=conan_toolchain.cmake -DCMAKE_EXPORT_COMPILE_COMMANDS=1

# Build the project (using the build system generated by CMake)
cmake --build Debug

# Change into the Debug directory, saving the previous directory on the stack
pushd Debug
