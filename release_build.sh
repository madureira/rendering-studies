...
conan install .. --output-folder=./Release --build=missing --settings=build_type=$BUILD_TYPE

# Run CMake to generate the build system in the current directory (build/)
cmake .. -B Release  -DCMAKE_BUILD_TYPE=$BUILD_TYPE -DCMAKE_TOOLCHAIN_FILE=conan_toolchain.cmake -DCMAKE_EXPORT_COMPILE_COMMANDS=1

# Build the project (using the build system generated by CMake)
cmake --build Release

# Change into the Release directory, saving the previous directory on the stack
pushd Release

# Execute the compiled binary ("rendering-studies" defined in the CMakeLists.txt)
./rendering-studies
